...
export interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
    className?: string;
    variant?: 'normal' | 'outline' | 'lightOutline' | 'custom';
  ...
  const classes = {
    ...
    normal:
      'bg-light text-dark border border-transparent hover:bg-afrogreen-hover',
    custom: 'border border-transparent',
    outline:
      'border border-border-400 bg-transparent text-body hover:text-light hover:bg-afrogreen hover:border-accent',
    lightOutline:
      'border-2 border-light bg-transparent hover:text-dark hover:bg-light hover:border-light',
    loading:
    ...
    (props, ref) => {
      ...
      const classesName = clsx(
        classes.root,
        {
          ...
          [classes.lightOutline]: !disabled && variant === 'lightOutline',
